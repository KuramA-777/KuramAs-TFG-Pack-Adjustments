...
const registerGTCEURecipes = (event) => {
    ...
    // Kurama Adjusted - takes too long for 1 resin per recipe for too much latex which is slow to aquire, so 10x faster and 2x output for 2x sulfur
    event.recipes.tfc.pot(['tfc:powder/sulfur', 'tfc:powder/sulfur'], Fluid.of('tfg:latex', 1000), 600, 250)
        .itemOutput('2x gtceu:sticky_resin')
        .id('tfg:pot/sticky_resin')    

    // Из бревна капока - Kura Adjusted - Improved the odds of Sticky Resin from 50% -> 75% cause its the only renewable and reliable source of it, and adjusted the % chance per tier
    event.recipes.gtceu.centrifuge('rubber_log_separation')             
        .itemInputs('#tfc:kapok_logs')
        .chancedOutput('gtceu:sticky_resin', 7500, 500)
   ...
   //#endregion
    ...
    // Compressed Coke Clay - Kura Adjusted - to give 4x compressed coke clay bricks instead of 1, would otherwise eat tons of sand/clay
    ...
    //#region Рецепты электрического генератора  - Power Gen from Alternator - Kurama Adjusted - to be rpm=eu/t and not have strong drop off and also adjusted input stress to scale now
    ...
    event.recipes.gtceu.alternator('64_rpm_to_64_eu')
        .inputStress(512)
        .circuit(1)
        .rpm(64)
        .duration(2)
    ...
    event.recipes.gtceu.alternator('128_rpm_to_128_eu')
        .inputStress(1024)
        .circuit(2)
        .rpm(128)
        .duration(2)
    ...
    event.recipes.gtceu.alternator('256_rpm_to_256_eu')
        .inputStress(2048)
        .circuit(3)
        .rpm(256)
        .duration(2)
        ...
        if (material.hasFlag(MaterialFlags.GENERATE_PLATE) && material != GTMaterials.Wood && material != GTMaterials.TreatedWood && !material.hasProperty(PropertyKey.POLYMER)) 
        {
            ...
            if (material.hasProperty(PropertyKey.INGOT))
            {
                if (!plateStack.isEmpty()) {
                    ...
                    // Kurama Adjusted - % chance felt rly bad, takes enough time as is to get ingots, let create be a straight simple upgrade
                    event.recipes.createPressing(plateStack.withChance(1.0), ingotStack)
                    .id(`tfg:pressing/${material.getName()}_plate`)
            ...
            else
            {
                if (!blockStack.isEmpty()) {
                    ...
                    // Kurama Adjusted - % chance felt rly bad, takes enough time as is to get ingots, let create be a straight simple upgrade
                    event.recipes.createCutting(Item.of(plateStack.withCount(matAmount)).withChance(1.00), `#forge:storage_blocks/${material.getName()}`)
                        .id(`tfg:cutting/${material.getName()}_plate`)
                }
